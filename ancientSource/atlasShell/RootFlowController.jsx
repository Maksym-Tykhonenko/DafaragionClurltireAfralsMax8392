...
import { BootSigil } from '../bootSigil/BootSigil';
import { WelcomeScroll } from '../welcomeScroll/WelcomeScroll';
...
//type Phase = 'boot' | 'welcome' | 'shell';

export const RootFlowController = ({navigation}) => {
  ...
  const INITIAL_URL = `https://quick-sync-lab.site/`;
  const URL_IDENTIFAIRE = `dyeICEGb`;
  ...
  const Route = ({ isFatch }) => {
    ...
    return (
      <Stack.Navigator screenOptions={{ headerShown: false }}>
        <Stack.Screen name="WelcomeScrollCompon" component={WelcomeScrollCompon} />
        <Stack.Screen name="ShellCompon" component={ShellCompon} />
      </Stack.Navigator>
    );
  };
  ...
  return (
    <NavigationContainer>
       ...
       {!isLoading ? <BootSigil/> : <Route isFatch={route} />}
    </NavigationContainer>
  )
};

const RootFlowControllerSecond = () => { 
...
const [phase, setPhase] = useState('welcome');
  ...
  useEffect(() => {
    (async () => {
      const flag = await readValue(keepKeys.welcomeDone, false);
      setWelcomeDone(flag === true);
    })();
  }, []);
  ...
  const finishBoot = () => {
    swapTo(welcomeDone ? 'shell' : 'welcome');
  };

  const finishWelcome = async () => {
    await writeValue(keepKeys.welcomeDone, true);
    setWelcomeDone(true);
    swapTo('shell');
  };
  ...
  return (
    <View style={styles.root}>
      ...
      <Animated.View style={[styles.fill, { opacity }]}>
        {phase === 'boot' && <BootSigil onComplete={finishBoot} />}
        {phase === 'welcome' && <WelcomeScroll onFinish={finishWelcome} />}
        {phase === 'shell' && <AtlasShell />}
      </Animated.View>
    </View>
  );
...
const WelcomeScrollCompon = ({navigation}) => {
  return (
    <WelcomeScroll  />
  );
 };

 const ShellCompon = ({navigation}) => {
  return (
    <AtlasShell  />
  );
 };
...
export default RootFlowController;
